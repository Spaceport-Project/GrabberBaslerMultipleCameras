...
void EncProc2 (NvEncoder *pEnc,  std::ifstream &fpIn, std::ofstream &fpOut)
    {
        int width = 4096;
        int height = 3000;
        ...
        oDeviceSrc = std::make_unique< npp::ImageNPP_8u_C1>(width, height, true);
        oDeviceDest = std::make_unique<  npp::ImageNPP_8u_C4> (width, height, true);
        	int nFrame = 0;
       ...
       while (true) {
            ...
            std::vector<std::vector<uint8_t>> vPacket;
                if (nRead == nFrameSize)
                {
                    ...
                    oDeviceDest->data(), 
                    0, (CUdeviceptr)encoderInputFrame->inputPtr,
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        // }
        std::vector<std::ofstream> fpOut;//(szOutFilePath, std::ios::out | std::ios::binary);
	
        for (int i = 0; i < nThread; i++) 
        {
            fpOut.push_back(std::ofstream(std::to_string(i)+".mp4", std::ios::out | std::ios::binary));
            vThread.push_back(NvThread(std::thread(EncProc2,
                vEnc[i].get(), 
                 std::ref(fpIn),
                 std::ref(fpOut[i])
               )));
