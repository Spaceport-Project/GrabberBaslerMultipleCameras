...
void EncodeCuda(int nWidth, int nHeight, NV_ENC_BUFFER_FORMAT eFormat, NvEncoderInitParam encodeCLIOptions, CUcontext cuContext, std::ifstream &fpIn, std::ofstream &fpOut)
{
	...
	oDeviceSrc = std::make_unique< npp::ImageNPP_8u_C1>(nWidth, nHeight, true);
    oDeviceDest = std::make_unique<  npp::ImageNPP_8u_C4> (nWidth, nHeight, true);
	while (true)
	{
		// Load the next frame from disk
		std::streamsize nRead = fpIn.read(reinterpret_cast<char*>(pHostFrame.get()), nFrameSize).gcount();
		// npp::ImageNPP_8u_C1 oDeviceSrc(nWidth, nHeight, true);
		oDeviceSrc->copyFrom(pHostFrame.get(), oDeviceSrc->pitch());
		...
		// npp::ImageCPU_8u_C4 oHostData(nWidth, nHeight);
		...
		if (nRead == nFrameSize)
		{
			...
			// void **tmpPtr = (void **)&(encoderInputFrame->inputPtr);
			// std::cout<<"ptr: "<< encoderInputFrame->inputPtr<< " "<< (void*)oDeviceDest.data()<<std::endl;
			// NvEncoderCuda::MovetoDeviceFrame(cuContext, oDeviceDest.data(), 0, tmpPtr,
			...
			// std::cout<<"encoderInputFrame->numChromaPlanes:"<<encoderInputFrame->numChromaPlanes<< " "<<encoderInputFrame->pitch<<" "<< encoderInputFrame->inputPtr<< " "<<(void*)oDeviceDest.data()<<std::endl;

			

			NvEncoderCuda::CopyToDeviceFrame(cuContext, oDeviceDest->data(), 0, (CUdeviceptr)encoderInputFrame->inputPtr,
				(int)encoderInputFrame->pitch,
				pEnc->GetEncodeWidth(),
				pEnc->GetEncodeHeight(),
...
void EncodeCudaOpInVidMem(int nWidth, int nHeight, NV_ENC_BUFFER_FORMAT eFormat, NvEncoderInitParam encodeCLIOptions, CUcontext cuContext, std::ifstream &fpIn, std::ofstream &fpOut, char *outFilePath, int32_t cuStreamType)
{
	...
	npp::ImageNPP_8u_C1 oDeviceSrc(nWidth, nHeight, true);
	npp::ImageNPP_8u_C4 oDeviceDest(nWidth, nHeight, true);
	...
	NppStreamContext nppStreamCtx;
	 ...
	 nppStreamCtx.hStream  = pCUStream->GetInputCUStream();
	// Encoding loop
	while (true)
	{
		// Load the next frame from disk
...
int main(int argc, char **argv)
{
	...
	std::string file_name =  "/home/spaceport/Softwares/GrabberBaslerMultipleCameras/build/bayer8.bin";
