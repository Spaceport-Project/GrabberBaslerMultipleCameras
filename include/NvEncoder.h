...
public:
    ...
    */
    virtual void EncodeFrame(std::vector<std::vector<uint8_t>> &vPacket,  uint64_t timestamp = 0, NV_ENC_PIC_PARAMS *pPicParams = nullptr);
    ...
    */
    int GetCapabilityValue(NVGUID guidCodec, NV_ENC_CAPS capsToQuery);
    ...
    */
    void CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams, NVGUID codecGuid, NVGUID presetGuid, NV_ENC_TUNING_INFO tuningInfo = NV_ENC_TUNING_INFO_UNDEFINED);
...
protected:
    ...
    *         NVENC hardware.
    */
    NVENCSTATUS DoEncode(NV_ENC_INPUT_PTR inputBuffer, NV_ENC_OUTPUT_PTR outputBuffer,  uint64_t timestamp, NV_ENC_PIC_PARAMS *pPicParams);
