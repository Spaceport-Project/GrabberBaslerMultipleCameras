...
#ifndef _WIN32
inline bool operator==(const NVGUID &guid1, const NVGUID &guid2) {
...
public:
    virtual NVGUID GetEncodeGUID() { return guidCodec; }
    virtual NVGUID GetPresetGUID() { return guidPreset; }
...
private:
    ...
    std::vector<std::string> tokens;
    NVGUID guidCodec = NV_ENC_CODEC_H264_GUID;
    NVGUID guidPreset = NV_ENC_PRESET_P3_GUID;
    NV_ENC_TUNING_INFO m_TuningInfo = NV_ENC_TUNING_INFO_HIGH_QUALITY;
    bool bLowLatency = false;
    
    const char *szCodecNames = "h264 hevc av1";
    std::vector<NVGUID> vCodec = std::vector<NVGUID> {
    ...
    std::vector<NVGUID> vPreset = std::vector<NVGUID> {
    ...
    std::vector<NVGUID> vH264Profile = std::vector<NVGUID> {
    ...
    std::vector<NVGUID> vHevcProfile = std::vector<NVGUID> {
        NV_ENC_HEVC_PROFILE_MAIN_GUID,
        NV_ENC_HEVC_PROFILE_MAIN10_GUID,
        NV_ENC_HEVC_PROFILE_FREXT_GUID,
    };
    const char *szAV1ProfileNames = "main";
    std::vector<NVGUID> vAV1Profile = std::vector<NVGUID>{
    ...
    const char *szProfileNames = "(default) auto baseline(h264) main(h264) high(h264) high444(h264)"
        ...
        " main(av1) high(av1)";
    std::vector<NVGUID> vProfile = std::vector<NVGUID> {
        NVGUID{},
